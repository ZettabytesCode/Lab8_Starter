describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const cartButton = await root.$('button');
    await cartButton.click();
    const buttonText = await cartButton.getProperty('innerText');
    const btnText = await buttonText.jsonValue();
    await cartButton.click();
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const cartCountInt = await cartCount.jsonValue();
    expect(cartCountInt).toBe("20");
  }, 10000);
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    const cartArray = await JSON.parse(cartJSON);
